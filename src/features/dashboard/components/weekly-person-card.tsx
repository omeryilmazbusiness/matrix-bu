...
const weeklyPerson = [
  {
    ...
    image: 'https://images.unsplash.com/photo-1568602471122-7832951cc4c5?w=800&h=1200&fit=crop',
    period: '1332-1406',
    likes: 3456,
    views: 15234
  },
...
export function WeeklyPersonCard() {
  ...
  const currentPerson = weeklyPerson[currentIndex];

  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentIndex((prev) => (prev + 1) % weeklyPerson.length);
    ...
    <div className="relative group h-full min-h-125 sm:min-h-150">
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 h-4/5">
          ...
          <div className="absolute inset-0 bg-linear-to-t from-black/90 via-black/60 via-30% via-black/40 via-50% via-black/20 via-70% to-transparent" />
          
          {/* Layer 2: Additional smoothing layer */}
          <div className="absolute inset-0 bg-linear-to-t from-black/50 via-black/25 via-40% to-transparent" />
          
          {/* Layer 3: Subtle blue tint for depth */}
          <div className="absolute inset-0 bg-linear-to-t from-gray-900/30 via-transparent to-transparent" />
